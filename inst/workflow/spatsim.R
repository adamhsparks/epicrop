...
getLandCells <-
  function(tablename, odbcname) {
    # tablename is database table name, usually nasa_1d, odbcname is usually geoclimate
    cnt <- 0
    repeat {
      cnt <- cnt + 1
    ...
    lc <-
      sqlQuery(
        con,
        paste(
          "SELECT cell FROM maskcells WHERE maskset_id =",
          maskset,
          " AND land = TRUE",
          sep = ""
        )
...
spatSimFlex <-
  function(region,
    ...
    if (!is.null(croppingraster) &
        class(croppingraster) == "RasterLayer") {
    ...
    result <-
      matrix(NA, nrow = length(cells), ncol = length(years) * mcount)
    ...
    for (cell in cells) {
      ...
      if ((cell - 1) %in% land) {
        ...
        wth@w$tavg[is.na(wth@w$tavg)] <-
          (wth@w$tmin[is.na(wth@w$tavg)] +
             wth@w$tmax[is.na(wth@w$tavg)]) /
          2
        wth@w$rhmax[is.na(wth@w$rhmax)] <-
          wth@w$rhmin[is.na(wth@w$rhmax)]
        cellresults <- numeric(0)

        if (checkcropping) {
          if (nosinglecrop & croppingraster[ccells == cell] > 1)
            PCISrice <- 1
          else
            PCISrice <- 0
        }
        ...
        for (d in 1:length(years)) {
          ...
          if (checkcropping)
            cellresults <- c(cellresults,
                             model(wth,
                                   emergence =
                                     pdate, PCISrice = PCISrice))
          else
    ...
    return(stack
           (paste(
             out, paste(colnames(result), ".tif", sep = ""), sep = "/"
           )))
  }
...
simulate.Region <-
  function(model,
    ...
    for (i in 1:ncell(aoiraster)) {
      #cell <- cells[i]
      if (is.na(plantingdate[pdcells[i]]) |
          plantingdate[pdcells[i]] < 1)
        next

      if (!is.null(ncroppings)) {
        if (nosinglecrop & ncroppings[nccells[i]] < 1)
          next
      }
      ...
      if (nrow(wth@w) > 0) {
        ...
        if (length(tavgcol) == 0)
          wth@w$tavg <-
            (wth@w$tmin + wth@w$tmax) / 2
        else
          colnames(wth@w)[tavgcol] <- "tavg"
        rhnx <-
          rhMinMax(wth@w$rh2m, wth@w$tmin, wth@w$tmax, wth@w$tavg)
        wth@w$rhmin <- rhnx[, 1]
        wth@w$rhmax <- rhnx[, 2]
        wth@w$prec[is.na(wth@w$prec)] <- 0
        wth@w$rhmin[is.na(wth@w$rhmin)] <- 0
        wth@w$tavg[is.na(wth@w$tavg)] <-
          (wth@w$tmin[is.na(wth@w$tavg)] + wth@w$tmax[is.na(wth@w$tavg)]) / 2
        wth@w$rhmax[is.na(wth@w$rhmax)] <-
          wth@w$rhmin[is.na(wth@w$rhmax)]
      } else {
        if (verbose) {
      ...
      for (d in 1:length(years)) {
        ...
        results <-
          rbind(results, c(i, years[d], model(wth, emergence = pdate)))
      }
    }

    regpre <-
      paste(xmin(region), xmax(region), ymin(region), ymax(region), sep = "_")
    for (y in years) {
      ...
      for (cc in 3:ncol(results)) {
        ...
        writeRaster(
          ...
          paste(
            outpath,
            paste(y, "_", regpre, "_", colnames(results)[cc], ".tif", sep = ""),
            sep = "/"
          ),
          format = "GTiff",
