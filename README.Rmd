...
This version provides easy to use functions to fetch weather data from NASA POWER, via the [_nasapower_](https://cran.r-project.org/package=nasapower) package (Sparks 2018, Sparks 2020) and predict disease severity of five rice diseases using a generic SEIR model (Zadoks 1971) function, `SEIR()`.
...
First you need to provide weather data for the model. _epicrop_ provides the `get_wth()` function to do this. Using it you can fetch weather data for any place in the world from 1983 to near present by providing the
