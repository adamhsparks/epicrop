...
A function, `get_wth()` is provided to simplify downloading weather data from the [https://power.larc.nasa.gov](NASA POWER web API) that are suitable for use in `epicrop`.
...
This is easily calculated from the output of any of the `predict_*()` functions or a custom `SEIR()` run using the _epifitter_ package (Alves and Del Pont 2020).

Load _eipfitter_ and simulate the disease severity in the dry season starting in January to compare with the initial `bb_wet` results.
...
Calculate the AUDPC using the `AUDPC()` from _epifitter_ for both seasons.
...
# References
Kaique dos S. Alves and Emerson M. Del Ponte (2020). epifitter: Analysis and Simulation of Plant Disease Progress Curves. R package version 0.2.0.  <https://CRAN.R-project.org/package=epifitter>.
